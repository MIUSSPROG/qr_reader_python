...
file = 'test.xlsx'  # название файла куда будет все сохраняться(должен быть в одном каталоге с испоняющим файлом)
    ...
    try: # для отлавливания ошибок
        ...
        if success: # если все прочитано успешно
            ...
            for code in decode(frame): # расшифровываем что считала камера
                ...
                if user_id not in saved_visitors_id: # если его еще нет с списке
                    if fio_map[user_id][3] == 0: # проверка если на листе с фио лимит 0, тогда ничего не делаем и оповещаем пользователя
                    ...
                    if (fio_map[user_id][3] - saved_visitors_map[user_id][4]) > 0: # проверяем, хвататет ли ему лимита на очередную регистрацию на мероприятие
                        ...
                        workbook[today][f'E{row + 2}'] = saved_visitors_map[user_id][4] + 1 # и в ячейку с толбце "посещение" добавляем +1
